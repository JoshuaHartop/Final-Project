...
void Player::SellItems(std::string sellChoice) {
	...
	if (it == Inventory.end()) {
		...
		if (Inventory[index].getAmount() == 1) {
			std::cout << "Your " << Inventory[index].getName() << " has been sold you have recieved " << Inventory[index].getMoney() << " gold.";
			addGold(AddGold);
			Inventory.erase(Inventory.begin() + index);
		}
		else if (Inventory[index].getAmount() >= 2) {
			std::cout << "Your " << Inventory[index].getName() << " has been sold you have recieved " << Inventory[index].getMoney() << " coins.";
			addGold(AddGold);
